...
class wechatCallbackapiTest
{
                    ...
                    $addAndResp=mysql_fetch_array($resultResp);
                        ...
                        else if($addAndResp['type']==1)
                        {
                         //tu wen
                            ...
                            while($count<sizeof($ss[$addAndResp['MsgFrom']]))
                            {
                                ...
                                <Url><![CDATA[http://182.92.228.241:8080/views/wechatAd/weChatPage.html]]></Url>
                                </item>";
                                $output.=$textAD;
                            $count++;
                            }              
                        ...
                        {//ad    
                            ...
                            while($count<sizeof($ss[$addAndResp['MsgFrom']]))
                            {
                                ...
                                <Description><![CDATA[".$ss[$addAndResp['MsgFrom']][$count]['Description']."]]></Description>
                                <PicUrl><![CDATA[".$ss[$addAndResp['MsgFrom']][$count]['PicUrl']."]]></PicUrl>
                                <Url><![CDATA[http://182.92.228.241:8080/views/wechatAd/advPage.html]]></Url>
                                </item>";
                                $output.=$textAD;
                            $count++;
                            }              
                        ...
                        {//third party
                            $party = mysql_query("select * from ThirdPartyDeveloper where wID='".$Aid['Aid']."'");
                            $partyurl=mysql_fetch_array($party);
                            thirdparty($partyurl['url']);
                        }
    ...
    private function thirdparty($thirdurl)
    {
    $xmlData = $GLOBALS["HTTP_RAW_POST_DATA"];//接收xml数据的文件
    $url = $thirdurl;
    //?signature='.$_GET["signature"].'&timestamp='.$_GET["timestamp"].'&nonce='$_GET["nonce"];  //接收xml数据的文件
